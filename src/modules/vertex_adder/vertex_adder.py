...
class VertexAdder(nn.Module):
	...
	def forward(self, batch_x, batch_c, batch_pid):
		...
		new_edge_index =  torch.Tensor(edge_index.size(0), 2*num_d_edges).type_as(edge_index).requires_grad_(False)
		...
		new_x = torch.zeros(x_prev.size(0), new_num_nodes, x_prev.size(2)).type_as(x_prev).requires_grad_(False)
		new_c = torch.zeros(c_prev.size(0), new_num_nodes, c_prev.size(2)).type_as(c_prev).requires_grad_(False)
		new_pid = torch.zeros(pid_prev.size(0), new_num_nodes, pid_prev.size(2)).type_as(pid_prev).requires_grad_(False)
