		...
		if self.params.rl_eval == True and self.params.rl_num_episodes:
			...
			for _  in range(episodes):
				...
				while not done:
					...
					edge_list1 = set([tuple(x) for x in data[1].edge_index.transpose(0,1).tolist()])
					data, reward, done, add_loss = self.splitter.split_and_reward(data, action, gt, gt_edges, gt_num_polygons)
					edge_list2 = set([tuple(x) for x in data[1].edge_index.transpose(0,1).tolist()])
					...
					if add_loss:
						count_add_loss += 1.0
						# print(f'DB2 Loss: {deformer_block.loss:.4f}, CLoss: {deformer_block.closs:.4f}, NLoss: {deformer_block.nloss:.4f}, ELoss: {deformer_block.eloss:.4f}, LapLoss: {deformer_block.laploss:.4f}')
					...
					utils.drawPolygons(utils.scaleBack(data[1].x), utils.scaleBack(gt[0]), gt_edges[0], proj_pred=proj_pred, proj_gt=proj_gt[0], color='red',out=self.params.expt_res_dir+f'/../train_out_deform_rl{episode_steps}.png',A=to_dense_adj(data[1].edge_index).cpu().numpy()[0], line=action[:4], text=f'Reward {reward}, Done {action[4]:4f}, Random: {self.params.start_steps > self.total_numsteps}')
			...
			print("----------------------------------------")
			print("Test Episodes: {}, Average Return: {}".format(episodes, round(avg_reward, 2)))
			print("----------------------------------------")
